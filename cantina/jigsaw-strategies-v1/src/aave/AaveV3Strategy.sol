...
contract AaveV3Strategy is IStrategy, StrategyBaseUpgradeable {
     ...
     * @notice A mapping that stores participant details by address.
         struct RecipientInfo {
            uint256 investedAmount;             deposit token amount
            uint256 totalShares;                atoken amount
        }
     */
    ...
    //~ halborn @audit-medium nonReentrant modifier should precedes all other modifiers
    /**
    ...
    function withdraw(
        ...
        params.shareRatio = OperationsLib.getRatio({
            ...
            //~ halborn @audit-low could allow users to withdraw slightly more assets than they should receive.
            //~ withdrawn amount could exceed the user's actual approval 
            //~ mitigation: OperationsLib.Rounding.Ceil
        ...
        // -------- calc the deposit token amount(investment) related to this withdraw shares
        ...
        //~ after deposit, we only know about atoken amount (shares), not deposit token amount
        //~ we need to use the shareRatio to calc the deposit token amount(investment) related to this shares
        ...
        // -------- Take protocol's fee from generated yield. 
    ...
    //~ halborn @audit-medium Missing access control
    //~ previous: function claimRewards( ) external override nonReentrant  
    //~ attacker can call the claimRewards() function on behalf of any user via the Strategy contract, instead of the StrategyManager contract
    /**
    ...
    function claimRewards(
        ...
        //~ Holding -> rewardsController(aave) -> claimAllRewards
        ...
        (, bytes memory returnData) = _genericCall({
            ...
            //~ 调用的目标函数是 claimAllRewards ，参数是 (eligibleTokens, _recipient)
            _call: abi.encodeCall(IRewardsController.claimAllRewards, (eligibleTokens, _recipient))
        });

        //~ q: 为什么 claimAllRewards的结果是一个list
        //~ a: 因为aave v3 的奖励token是多样的
