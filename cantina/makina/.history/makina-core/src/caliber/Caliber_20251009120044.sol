    ...
    ICaliber
{
    ...
    struct CaliberStorage {
        address _hubMachineEndpoint;                              //~ mailbox（spoke Caliber） Machine（hub Caliber）
    ...
    function getDetailedAum() external view override returns (uint256, bytes[] memory, bytes[] memory) {
        //~ qa: 这里怎么这样检查重入？而且是 view？ 
        //~ 禁止 nonReentrant 函数执行过程中去调用它，防止读到不一致的中间状态
    ...
    //~ @audit 创建或操作Position时，有没有检查caliber中是不是有足够的资金支持操作？ 
    /// @inheritdoc ICaliber
    ...
    //~ @audit machine 跨过来，但 caliber 没有用作 position ，也没有在 caliberMailbox 中作为跨出的资金，没有被算入 aum
    /// @inheritdoc ICaliber
