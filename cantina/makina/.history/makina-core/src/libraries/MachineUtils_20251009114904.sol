...
library MachineUtils {
    ...
    /* 
    根据经过的时间计算并收取三种费用：
    管理费（Management Fees）、安全模块费（Security Module Fees）和绩效费（Performance Fees），
    通过增发份额代币（share token）的方式实现费用收取
    */
    ...
    function _getTotalAum(Machine.MachineStorage storage $, address oracleRegistry) private view returns (uint256) {
        ...
        for (uint256 i; i < len; ++i) {
            ...
            // -------- 汇总各个 spoke caliber 在其链上转移到对应 hub machine 的 senting 资金（在 hub machine 中）
            ...
            // -------- 汇总所有 spoke calibers 想跨链到其它链 machine 的 senting 资金（在CaliberMailbox 中）
