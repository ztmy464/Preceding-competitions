...
contract Distributor is IDistributor, OwnableUpgradeable, UUPSUpgradeable {
    ...
    /* 
        distributedRewards
        │
        ├── #1 RewardPool （PublicRewardPool）
        │   ├── DepositPoolA (0xAAA...) : 1000
        │   └── DepositPoolB (0xBBB...) : 500
        │
        └── #2 RewardPool （PrivateRewardPool）
            └── DepositPoolC (0xCCC...) : 10000
     */
    ...
    /**********************************************************************************************/
    //~ ✅
    //~ 提供最新代币价格，用于公共池的奖励按比例计算。
    //~ 确保分配奖励时，每个 deposit pool 的收益计算是基于 最新市场价值
    ...
    /**********************************************************************************************/
    // ~
    // ~存款  并触发收益计算的核心函数，确保每次存款前的历史收益不会丢失，同时支持不同策略
    ...
    //~ ✅
    //~ 按Yield比例 分配分发 reward pool 的奖励(update distributedRewards)
    function distributeRewards(uint256 rewardPoolIndex_) public {
        ...
        if (!rewardPool_.isRewardPoolPublic(rewardPoolIndex_)) {
            ...
            //~ 私有池在创建时规定只有一个 deposit pool，只分配给 唯一 deposit pool（索引 0）
        ...
        //~ the operation will return early without distributing rewards,
        //~ but `rewardPoolLastCalculatedTimestamp` has already been updated.

        //~ an attacker repeatedly calls distributeRewards
        //~ before minRewardsDistributePeriod has passed, updating
        //~ `rewardPoolLastCalculatedTimestamp` and preventing proper reward distribution.
        ...
        //~ 没有任何 depositPool 在这一轮产生收益, 把这些奖励暂时累加到 undistributedRewards 里
        if (totalYield_ == 0) {
        ...
        //~ Reward 是 协议奖励池提供的额外代币，可以看作是 “平台激励”。
        //~ Yield 是 用户存入策略的真实市场收益，可以看作是 “投资收益”。
        //~ 最终发给用户的收益 = Yield + 按比例分配的 Reward。

        //~ PublicRewardPool 里面有多个 deposit pool
        //// Calculate `depositPools` shares and reward amount for each `depositPool`
        for (uint256 i = 0; i < length_; i++) {
            if (yields_[i] == 0) continue;
            //~ 根据每个 deposit pool 的收益占比分配 rewards_
    ...
    //~ 由治理 (owner) 手动处理 undistributedRewards
    function withdrawUndistributedRewards(address user_, address refundTo_) external payable onlyOwner {
        ...
        //~ 通过 LayerZero 发一条跨链消息，通知 L2 的接收合约给用户地址 user_ 铸造等量的代币
