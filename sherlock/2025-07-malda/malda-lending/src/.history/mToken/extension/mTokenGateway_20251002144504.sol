...
contract mTokenGateway is OwnableUpgradeable, ImTokenGateway, ImTokenOperationTypes {
    ...
    // ~ @audit-M Issue M-11: There is no endpoint for triggering liquidateExternal from extension chain to be executed by proof forwarder
    ...
    function supplyOnHost(uint256 amount, address receiver, bytes4 lineaSelector)
        ...
        //~ @audit-M outHere 函数上可以绕过黑名单
        //~ receiver value passed to the endpoint is overwritten by _sender in the journal data
        ifNotBlacklisted(receiver)
    {
        // checks
        require(amount > 0, mTokenGateway_AmountNotValid());
        //~ 除了本链 gas，还要支付额外的 gasFee，用于消息跨链传输 & 在 host 上执行 mintExternal 的费用
    ...
    function _outHere(bytes memory journalData, uint256 amount, address receiver) internal {
        ...
        require(amount > 0, mTokenGateway_AmountNotValid());
        //~ qa: 提现额amount小于_accAmountOut 主链生成的用户可提取金额上限证明不就行了吗，为什么还要减去accAmountOut[_sender] 
        //~ a 如果仅验证 _accAmountOut >= amount ，用户可能会重复使用同一个 journalData 多次提现，导致超额提取
        //~ gateway合约必须记录已提取金额状态，用于验证剩余额度
        
        //~ _accAmountOut 主链生成的用户可提取金额上限证明 
