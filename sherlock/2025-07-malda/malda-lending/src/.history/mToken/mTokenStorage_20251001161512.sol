...
abstract contract mTokenStorage is ImToken, ExponentialNoError {
    ...
    //~ 协议管理的总资产规模 (totalCash + totalBorrows - totalReserves) 
    /**
     * @inheritdoc ImTokenMinimal
     */
    ...
    function _exchangeRateStored() internal view virtual returns (uint256) {
        ...
        if (_totalSupply == 0) {
             ...
             *  分子 (totalCash + totalBorrows - totalReserves) 表示协议控制的净基础资产总量，
             *  分母 totalSupply 表示用户持有的mToken总量。
             *  两者的比值就是mToken的内在价值，随着利息累积，这个汇率会逐渐增加，从而使用户持有的mToken价值增长
             */
