...
abstract contract mTokenStorage is ImToken, ExponentialNoError {
    ...
    //~ 系统维护一个全局增长的 borrowIndex （借款利息累积指数）
    /**
     * @inheritdoc ImToken
     */
    ...
    //~ 协议持有的准备金总额（以基础资产计价）通常从借款利息中按一定比例提取累积
    /**
     * @inheritdoc ImToken
     */
    ...
    /**
     * @notice Calculates the exchange rate from the underlying to the MToken (the value of Mtoken to underlying)
    ...
    function _exchangeRateStored() internal view virtual returns (uint256) {
        ...
        if (_totalSupply == 0) {
             ...
             *  分子 (totalCash + totalBorrows - totalReserves) 表示协议账上的净基础资产总量
             *  分母 totalSupply 表示用户持有的mToken总量。
             *  两者的比值就是mToken的内在价值，随着利息累积，这个汇率会逐渐增加
             */
            //~ @audit-H Veridise 2: 
            //~ 使用 IERC20(underlying).balanceOf(address(this)) 来获得 underlying tokens 的总量 而不是 维护一个 totalUnderlying 全局变量
            //~ 由于 host 链上不存放全部资金 余额不能反映 underlying tokens 的总量
     ...
     */
    function _doTransferIn(address from, uint256 amount) internal virtual returns (uint256); //~ mErc20.sol 
    ...
    function _accrueInterest() internal {
        ...
        //~ @audit 从 JumpRateModelV4 的实现可见，年利率参数被转换为 每区块利率，但该值在计算利息时直接与 blockDelta 时间差相乘，这是不是不对

        //~ q: Calculate the time elapsed since the last accrual instead of the number of blocks elapsed ?
        ...
        // ~ @audit 为什么最后要加 borrowIndexPrior
