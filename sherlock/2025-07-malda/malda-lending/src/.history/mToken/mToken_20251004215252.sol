...
abstract contract mToken is mTokenConfiguration, ReentrancyGuard {
    ...
    function _borrowBalanceStored(address account) internal view returns (uint256) {
         ...
         *  recentBorrowBalance = borrower.borrowBalance * borrowIndex / Snapshot.borrowIndex
         */
        uint256 principalTimesIndex = borrowSnapshot.principal * borrowIndex;
        //~ BorrowBalance = principal * borrowIndex / borrower.interestIndex 
    ...
    function __liquidate(
        ...
        //~ liquiditor can't repay all(close factor)
        require(borrower != liquidator, mt_InvalidInput());
        ...
        // --------- take the liquidator's mTokenCollateral(this) away
        ...
        //~ seizeTokens (amount of mTokenCollateral) = 
        ...
        //~ @audit does this impede the bad debt liquidation? 没有滑点保护（确保有利润）
        /* Revert if borrower collateral token balance < seizeTokens */
        require(ImToken(mTokenCollateral).balanceOf(borrower) >= seizeTokens, mt_LiquidateSeizeTooMuch());

        // --------- seize mTokenCollateral from borrower to liquidator
    ...
    {   
        /* 
        redeemTokensIn：用份额数量来赎回（输入多少 Vault Token，换多少资产）。
        redeemAmountIn：用目标资产数量来赎回（想拿回多少资产，Vault 算出要销毁多少份额）。
         */
        //~ 只用 份额 或 资产 的一种输入，不能同时指定（ ❌ 两个都大于 0）
        require(redeemTokensIn == 0 || redeemAmountIn == 0, mt_InvalidInput());

        //~ Calculates the exchange rate from the underlying to the MToken (the value of Mtoken to underlying)
        /* exchangeRate = invoke Exchange Rate Stored() */
        Exp memory exchangeRate = Exp({mantissa: _exchangeRateStored()});
        // ---------- calc mtoken amount(redeemTokens) and underlying amount(redeemAmount)
        ...
        if (redeemTokensIn > 0) {
            ...
            //~ @audit-M Issue H-2: 如果汇率变大，赎回的四舍五入方向错误可能会导致耗尽
            ...
            //~ mitigation: redeemTokens = divUp_(redeemAmountIn, exchangeRate);  
    ...
    function __mint(address minter, address receiver, uint256 mintAmount, uint256 minAmountOut, bool doTransfer)
        private
    {
        // ------ Operator Defense      reward distribution for liquidity providers.
        ...
        // ------ chack underlying token amount < supplyCap
        /* We call the defense hook */
        IOperatorDefender(operator).afterMTokenMint(address(this));

        // ------ Activate market for the entered user
