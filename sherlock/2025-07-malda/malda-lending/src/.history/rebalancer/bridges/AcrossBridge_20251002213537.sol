...
contract AccrossBridge is BaseBridge, IBridge, ReentrancyGuard {
    ...
    //~ @audit-M Issue M-16: 有的资产across 、Everclear 桥不支持
    //~ https://docs.across.to/reference/api-reference#get-swap-tokens
    //~ https://docs.everclear.org/resources/contracts/mainnet#registered-assets
    // ----------- EXTERNAL ------------
    /**
     * @inheritdoc IBridge
     */
    ...
    function _depositV3Now(bytes memory _message, address _token, uint32 _dstChainId, address _market) private {
        ...
        //~ @audit-M Issue M-15: If Across Bridging fails, all funds intended for bridging will become locked
        //~ According to the Across documentation, if a deposit expires or a fill fails, funds are refunded to the depositor on the origin chain.
