...
contract EverclearBridge is BaseBridge, IBridge {
    ...
    function sendMsg(
        ...
        //~ @audit-M Issue H-1: Rebalancer can steal funds from markets by sending to custom receiver through Everclear Bridge 
        //~ CCC calldata 内容未被检查
        ...
        //~ @audit-M NOTE: Issue M-5: Rebalancer can send to unallowed destination chains through EverclearBridge
        //~ the intent is created on Everclear using all the provided destinations, and can be fulfilled on any of the chains of the list. 
        //~ If it is fulfilled on a chain that Malda does not handle, the funds are lost. 
        bool found;
        ...
        //~ @audit-M NOTE: Issue M-6: EverclearBridge does not pull tokens from the Rebalancer, causing all rebalancing operations to fail

        //~ mitigation: retrieve tokens from `Rebalancer`
        ...
        //~ @audit-M Issue M-9 EverclearBridge 只批准了 amount，但 FeeAdapter 试图拉取 amount + fee
        //~ mitigation: params.amount + params.feeParams.fee
        require(params.receiver != bytes32(0), Everclear_AddressNotValid());
        SafeApprove.safeApprove(params.inputAsset, address(everclearFeeAdapter), params.amount + params.feeParams.fee);
        //~ @audit-M NOTE: Issue M-10: Unenforced maxFee and ttl Parameters in sendMsg Function
        //~ @audit-M Issue M-12: Rebalancer can drain market funds via excessive bridge fees
