...
contract Rebalancer is IRebalancer {
    ...
    function sendMsg(address _bridge, address _market, uint256 _amount, Msg calldata _msg) external payable {
        //~ The REBALANCER_EOA role is considered semi-trusted 
        //~ assumption that the Rebalancer "cannot transfer user funds" and can only perform DDoS-style attacks 
        // checks
        ...
        //~ @audit-M Issue M-7: Incorrect transfer size validation after time window reset can lead to rebalancing DoS
        //~ mitigation: currentTransferSize[_msg.dstChainId][_msg.token].size < _maxTransferSize
