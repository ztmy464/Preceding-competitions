...
library BorrowLogic {
        ...
        returns (uint256 repaid)
    {
        ...
        // ---------- try repay interest
        //~ debt should < total agent need repaid since of interest
        ...
        // ---------- use the remain to repay debt
        uint256 vaultRepaid = Math.min(remaining, reserve.debt);

        // ---------- repay unrealizedInterest
        if (restakerRepaid > 0) {
    ...
    //~ RestakerInterest 是 accured ，然后增加债务给 borrower
    //~ Interest 是 IERC20(reserve.debtToken).totalSupply() - reserve.debt，是 debtToken updateIndex 机制产生的

    //~ q: 来在repay之前调用它累计利息呢？
    //~ 机器人，因为不是累计利息，该函数实现了在 borrower 实际还款前提前实现利息的机制
        ...
        returns (uint256 realizedInterest)
    {
        ...
        //~ 利息随时可以实现，没 repay 就从金库借，repay 了就是取出来
        ...
        returns (uint256 realization)
    {
        ...
        if (totalDebt > vaultDebt + totalUnrealizedInterest) {
            //~ totalUnrealizedInterest 是 staker 的未实现利息，应该优先于利息被保证
        ...
        returns (uint256 realization, uint256 unrealizedInterest)
    {
        ...
        //~ totalSupplies[_asset] - $.totalBorrows[_asset]
        uint256 reserves = IVault(reserve.vault).availableBalance(_asset);
