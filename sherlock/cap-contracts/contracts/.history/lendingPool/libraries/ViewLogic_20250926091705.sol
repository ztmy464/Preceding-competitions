...
library ViewLogic {
    ...
    {
        //~ totalDelegation Total delegation of an agent in USD
        //~ skip: how does totalDelegation be calc?
        ...
        returns (uint256 maxLiquidatableAmount)
    {
        ...
        //~ health --> totalDelegation * liquidationThreshold / totalDebt > $.targetHealth
        ...
        )
    {
        ...
        //~ emergencyHealth -->  totalDelegation * $.emergencyLiquidationThreshold / totalDebt  < health
    ...
    function bonus(ILender.LenderStorage storage $, address _agent) internal view returns (uint256 maxBonus) {
        ...
        if (totalDelegation > totalDebt) {
            //~ totalDelegation * $.emergencyLiquidationThreshold / totalDebt --> emergencyHealth < health
            // Emergency liquidations get max bonus
            if (totalDelegation * $.emergencyLiquidationThreshold / totalDebt < 1e27) {
                maxBonus = $.bonusCap;   
            } else {
                //~ eg: $.grace = 86400（宽限期1天） 
                //~ eg: $.expiry = 172800（清算窗口2天）
